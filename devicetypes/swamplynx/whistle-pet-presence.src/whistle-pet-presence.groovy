...
metadata {
	definition (name: "Whistle Pet Presence", namespace: "swamplynx", author: "SwampLynx", cstHandler:true, mnmn:"SmartThings", vid:"generic-arrival") {
...
def installed() {
    log.info("Whistle DTH Installed")
     initialize()
}

def updated() {
    log.info("Whistle DTH Updated")
    initialize()
}

def initialize() {
    log.info("Whistle DTH - Initializing Health Check and Scheduled Poll")
    sendEvent(name: "DeviceWatch-DeviceStatus", value: "online")
    sendEvent(name: "healthStatus", value: "online")
    sendEvent(name: "DeviceWatch-Enroll", value: [protocol: "cloud", scheme:"untracked"].encodeAsJson(), displayed: false)
    log.debug "Whistle DTH - Health Check estsablished"
    runEvery1Minute (refresh)
    log.debug "Whistle DTH - Data scheduled to poll cloud every minute" 
}

def refresh() { 
	 log.info("Whistle Presence - Refresh or Scheduled Refresh Requested")
    callAPI()
}
